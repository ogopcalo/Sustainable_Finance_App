...
# Optional lock for safer matplotlib rendering in deployed/shared apps
_lock = RLock()

st.set_page_config(page_title="Efficient Frontier App", layout="wide")

st.title("Efficient Frontier for Two Assets")
st.write("Explore how expected return, volatility, correlation, and the risk-free rate affect the portfolio frontier.")
...
mu1 = st.sidebar.slider("Expected return: Asset 1", 0.00, 0.30, 0.05, 0.01)
mu2 = st.sidebar.slider("Expected return: Asset 2", 0.00, 0.30, 0.12, 0.01)

sigma1 = st.sidebar.slider("Volatility: Asset 1", 0.01, 0.60, 0.09, 0.01)
sigma2 = st.sidebar.slider("Volatility: Asset 2", 0.01, 0.60, 0.20, 0.01)

rf = st.sidebar.slider("Risk-free rate", 0.00, 0.10, 0.02, 0.005)

available_rhos = [-1.0, -0.8, -0.5, -0.2, 0.0, 0.2, 0.5, 0.8, 1.0]
selected_rhos = st.sidebar.multiselect(
